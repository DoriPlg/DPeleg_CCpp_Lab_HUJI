...
#define ARROW " ->"
#define LADDER " -ladder to->"
#define SNAKE " -snake to->"

enum {SEED = 1, SENTENCES};
...
int fill_database_snakes(MarkovChain *markov_chain)
{
    ...
    for (size_t i = 0; i < BOARD_SIZE; i++)
    {
        ...
        {
            index_to = MAX(\
                    cells[i]->snake_to,cells[i]->ladder_to) - 1;
 ...
 */
bool game_finished(const void *data)
{
    Cell *cell = (Cell*) data;
 ...
 * @param data1
 * @param data2
 * @return the comparison between the two cells
 */
int compare_cell(const void *data1, const void *data2)
{
...
/*
*copies a cell to a new allocated cell
* @param data
* @return a new allocated cell
*/
void* copy_cell(const void *data)
{
    ...
    *new_cell = (Cell) {cell->number, cell->ladder_to, cell->snake_to};
    return new_cell;
}
...
int main(int argc, char *argv[])
{
    ...
    unsigned int seed = strtol(argv[SEED],NULL,BASE), \
        walks = strtol(argv[SENTENCES],NULL,BASE);
    srand(seed);
    ...
    (*markov_chain) = (MarkovChain)\
        {NULL, print_cell, compare_cell, free_cell, copy_cell, game_finished};
