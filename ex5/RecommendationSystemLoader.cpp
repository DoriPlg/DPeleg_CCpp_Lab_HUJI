...
typedef std::unique_ptr<RecommendationSystem> sp_rs;

sp_rs RecommendationSystemLoader::create_rs_from_movies(const\
                        std::string &movies_file_path) noexcept(false)
{
    sp_rs rs = \
            std::make_unique<RecommendationSystem>(RecommendationSystem());
